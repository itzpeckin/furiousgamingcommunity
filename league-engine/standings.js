(function initStandingsService(global){
  ...
  function poolSeason(){
    ...
    const rows=board.map(r=>{const weeks=(r.weeks||[]).slice();const scoredWeeks=weeks.filter(w=>Number(w.finalGames)>0);const bestWeek=weeks.reduce((m,w)=>Math.max(m,Number(w.points)||0),0);const avg=scoredWeeks.length?scoredWeeks.reduce((s,w)=>s+(Number(w.points)||0),0)/scoredWeeks.length:0;const localRemaining=Object.values(entryConfig.entries?.[r.userId]?.picks||{}).filter(p=>!finalIds.has(p.gameId)).reduce((s,p)=>s+(Number(p.confidence)||0),0);const graded=Number(r.gradedPicks)||Math.max(0,Number(r.correctPicks||0)+Number(r.incorrectPicks||0));return{...r,gradedPicks:graded,incorrectPicks:Number(r.incorrectPicks)||Math.max(0,graded-Number(r.correctPicks||0)),correctPercentage:Number.isFinite(Number(r.correctPercentage))?Number(r.correctPercentage):(graded?Number(((Number(r.correctPicks||0)/graded)*100).toFixed(1)):0),weeksWon:0,bestWeek,averageWeeklyScore:Number(avg.toFixed(1)),remainingPossiblePoints:Number.isFinite(Number(r.remainingPossiblePoints))?Number(r.remainingPossiblePoints):localRemaining,submitted:r.status==='submitted'}});
  ...
  function poolWeek(week){const rows=poolSeason().map(r=>{const w=(r.weeks||[]).find(x=>Number(x.week)===Number(week))||{points:0,correct:0,gradedPicks:0,incorrectPicks:0,finalGames:0};const graded=Number(w.gradedPicks)||Math.max(0,Number(w.correct||0)+Number(w.incorrectPicks||0));return{userId:r.userId,name:r.name,teamId:r.teamId,status:r.status,points:Number(w.points)||0,correctPicks:Number(w.correct)||0,gradedPicks:graded,incorrectPicks:Number(w.incorrectPicks)||Math.max(0,graded-Number(w.correct||0)),correctPercentage:Number.isFinite(Number(w.correctPercentage))?Number(w.correctPercentage):(graded?Number(((Number(w.correct||0)/graded)*100).toFixed(1)):0),finalGames:Number(w.finalGames)||0}}).sort((a,b)=>b.points-a.points||b.correctPicks-a.correctPicks||String(a.name).localeCompare(String(b.name)));return freeze(clone(rows.map((r,i)=>({...r,rank:i+1,week:Number(week)}))))}
