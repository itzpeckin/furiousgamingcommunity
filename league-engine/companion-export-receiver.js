(() => {
  ...
  function discoveryEndpoint(){ return endpoint().replace(/\/export$/, '/discovery'); }
  ...
  async function refresh(options = {}) {
    lastError = null;
    try {
      const [status, history, discovery] = await Promise.all([
        ...
        fetch(discoveryEndpoint(), { headers: { accept: 'application/json' }, cache: 'no-store' }).then(readJson)
      ]);
      latestStatus = freeze({ ...status, checkedAt: new Date().toISOString() }); latestHistory = freeze(history); latestDiscovery = freeze(discovery);
  ...
  function getDiscovery(){ return clone(latestDiscovery); }
  ...
  function exportUrlWithToken(token){ const base=new URL(endpoint(),location.origin).toString().replace(/\/$/,''); const value=String(token||'').trim(); return value ? `${base}/${encodeURIComponent(value)}` : `${base}/YOUR_PRIVATE_TOKEN`; }
  function copyUrl(token=''){ const url=exportUrlWithToken(token); return navigator.clipboard?.writeText(url).then(()=>url).catch(()=>url); }
  function diagnostics(){ return freeze({service:'leagueCompanionExportReceiver',version:VERSION,leagueId:tenant().current().id,leagueSlug:tenant().current().slug,endpoint:endpoint(),historyEndpoint:historyEndpoint(),discoveryEndpoint:discoveryEndpoint(),method:'POST',authentication:'path-token',catchAllRoutes:true,durableD1History:true,privateR2Payloads:true,kvLatestPointer:true,duplicateProtection:'sha-256',rejectionRetainsRawPayload:true,automaticActivation:false,statusAvailable:Boolean(latestStatus),historyAvailable:Boolean(latestHistory),inspectionAvailable:Boolean(latestInspection),rawPayloadExposed:false,lastError}); }
  ...
  function renderDiscovery(){
    const routes=latestDiscovery?.routes||[];
    if(!routes.length)return'<p class="league-import-status-note">No real Madden Companion dataset routes have been captured yet.</p>';
    return `<div class="import-history-list">${routes.slice(0,20).map(route=>`<div class="import-history-row"><div><strong>${escapeHtml(route.routePath)}</strong><span>${route.latestReceivedAt?new Date(route.latestReceivedAt).toLocaleString():'—'} · ${formatBytes(route.latestByteLength)}</span></div><div><span class="pill pill--success">Captured</span><span>${route.captureCount} request${route.captureCount===1?'':'s'} · ${(route.topLevelKeys||[]).length} top-level fields</span></div></div>`).join('')}</div>`;
  }
  function renderPanel(){
    ...
    return `<article class="card companion-export-receiver-card" data-companion-export-receiver-panel>
      ...
      <div class="league-import-framework-actions"><button class="button button--primary" data-refresh-companion-receiver>Check Discovery</button><button class="button button--ghost" data-copy-companion-endpoint>Copy Tokenized Base URL</button></div>
      ...
      <h4>Discovered Madden Routes</h4>${renderDiscovery()}<h4>Recent Receiver History</h4>${renderHistory()}
  ...
  window.addEventListener('franchisehq:league-tenant-changed',()=>{latestStatus=null;latestHistory=null;latestInspection=null;latestDiscovery=null;lastError=null;rerender()});
  if(!HQ.defineModuleService)throw new Error('platform/core.js must load before companion-export-receiver.js.');
  HQ.defineModuleService('league','leagueCompanionExportReceiver',{endpoint,inspectEndpoint,historyEndpoint,discoveryEndpoint,refresh,inspect,reject,getStatus,getInspection,getDiscovery,subscribe,exportUrlWithToken,copyUrl,renderPanel,diagnostics},{replace:true,alias:'leagueCompanionExportReceiver'});
