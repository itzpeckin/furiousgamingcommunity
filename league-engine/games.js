(function initGamesService(global){
  ...
  function setSubmissionWindow(startWeek,endWeek){const s=ensure(),c=s.seasons[seasonId()],maxWeek=Math.max(1,...games().map(w=>Number(w.week)||0)),start=Math.max(1,Math.min(maxWeek,Number(startWeek)||1)),end=Math.max(start,Math.min(maxWeek,Number(endWeek)||maxWeek));c.status='open';c.openWeeks=Array.from({length:end-start+1},(_,i)=>start+i);c.openedAt=new Date().toISOString();write(s);return config()}
  function closeSubmissionWindow(){const s=ensure(),c=s.seasons[seasonId()];c.status='locked';c.openWeeks=[];c.lockedAt=new Date().toISOString();write(s);return config()}
  function isWeekOpen(week,userId=account().id){const c=ensure().seasons[seasonId()],e=c.entries[userId];return c.status==='open'&&c.openWeeks.includes(Number(week))&&!e?.submittedWeeks?.[String(week)]}
  ...
  function clearWeek(week,userId=account().id){const s=ensure(),c=s.seasons[seasonId()];if(!isWeekOpen(week,userId))return{ok:false,error:`Week ${week} is not currently open for picks.`};const e=baseEntry(c,userId);Object.keys(e.picks).forEach(id=>{if(Number(e.picks[id]?.week)===Number(week))delete e.picks[id]});e.status='draft';e.submittedAt=null;e.updatedAt=new Date().toISOString();c.entries[userId]=e;write(s);return{ok:true,entry:entry(userId)}}
  function clearSeason(userId=account().id){const s=ensure(),c=s.seasons[seasonId()];if(c.status!=='open')return{ok:false,error:'Confidence Pool submissions are locked.'};c.entries[userId]={userId,season:seasonId(),status:'draft',picks:{},submittedWeeks:{},updatedAt:new Date().toISOString(),submittedAt:null};write(s);return{ok:true,entry:entry(userId)}}
  ...
  function autoAssign(week,userId=account().id){
    const s=ensure(),c=s.seasons[seasonId()];if(!isWeekOpen(week,userId))return{ok:false,error:`Week ${week} is not currently open for picks.`};
  ...
  function validateWeek(week,userId=account().id){const e=entry(userId),w=getWeek(week),issues=[],vals=[];w.games.forEach(g=>{const p=e.picks[g.id];if(!p?.selectedTeamId)issues.push(`Missing winner for ${g.id}`);if(!Number.isInteger(Number(p?.confidence)))issues.push(`Missing confidence for ${g.id}`);else vals.push(Number(p.confidence))});if(new Set(vals).size!==vals.length)issues.push(`Duplicate confidence values in Week ${week}`);const expected=new Set(w.games.map((_,i)=>i+1));if(vals.length===w.games.length&&vals.some(v=>!expected.has(v)))issues.push(`Confidence values must use 1 through ${w.games.length}`);return freeze({week:Number(week),valid:issues.length===0,issues,totalGames:w.games.length,picked:w.games.filter(g=>e.picks[g.id]?.selectedTeamId).length,submitted:Boolean(e.submittedWeeks?.[String(week)])})}
  function submitWeek(week,userId=account().id){if(!isWeekOpen(week,userId))return{ok:false,error:`Week ${week} is not currently open for submission.`};const valid=validateWeek(week,userId);if(!valid.valid)return{ok:false,error:`Complete every Week ${week} game and use each confidence value once.`,validation:valid};const s=ensure(),c=s.seasons[seasonId()],e=baseEntry(c,userId);e.submittedWeeks[String(week)]=new Date().toISOString();const allWeeks=games().every(w=>e.submittedWeeks[String(w.week)]);e.status=allWeeks?'submitted':'partial';e.submittedAt=allWeeks?new Date().toISOString():e.submittedAt||null;e.updatedAt=new Date().toISOString();c.entries[userId]=e;write(s);return{ok:true,entry:entry(userId),week:Number(week)}}
  ...
  function diagnostics(){const c=config();return freeze({service:'games',version:'5.6.1a',season:c.season,poolStatus:c.status,gameCount:allGames().length,currentWeek:currentWeek(),entryCount:Object.keys(c.entries).length,ownerRatingBaseline:BASELINE,ownerRankingCount:ownerHistory().length})}
