(() => {
  ...
  function changesRequested(workflow){return workflow?.status==='rejected'&&workflow?.review?.result==='rejected'}
  ...
  function tradeCard(workflow,tab){
    ...
    const tally=workflow.review||{approvals:0,rejections:0};
    return `<article class="card live-trade-card live-trade-card--premium"><div class="live-trade-card__head"><div><span class="pill pill--${tone}">${esc(statusLabel(workflow.status))}</span><h3>${esc(workflowTitle(workflow,tab))}</h3><small>${esc(date(workflow.updatedAt))}</small><span class="live-trade-vote-tally"><b>${Number(tally.approvals||0)} approval${Number(tally.approvals||0)===1?'':'s'}</b><b>${Number(tally.rejections||0)} rejection${Number(tally.rejections||0)===1?'':'s'}</b></span></div><div class="live-trade-card__actions"><button class="button button--primary" data-live-open-trade="${esc(workflow.id)}">${workflowActionLabel(workflow,tab)}</button>${withdrawControls(workflow)}</div></div><div class="trade-package-matchup trade-package-matchup--review">${received}</div>${fairness?`<div class="trade-package-value">${fairness.rows.map(row=>`<button type="button" data-live-open-package-value="${esc(row.teamKey)}" data-live-package-direction="received" data-live-value-trade="${esc(workflow.id)}"><small>${esc(teamAbbr(row.teamKey))} package value</small><strong>${row.receivedValue.toLocaleString()} ⓘ</strong></button>`).join('')}<button type="button" data-live-open-fairness-value data-live-value-trade="${esc(workflow.id)}" class="${fairness.overall>=91?'is-positive':''}"><small>Package balance</small><strong>${fairness.overall}% · ${fairness.overall>=91?'Fair value':'Review value'} ⓘ</strong></button></div>`:''}</article>`;
  }
  ...
  function renderDetail(workflow){
    ...
    const revisionRequested=changesRequested(workflow);
    const mayRevise=participant&&((workflow.status==='negotiating'&&(proposer||!ownAcceptance))||revisionRequested);
  ...
  function renderTradeBlock(){
    ...
    const cards=visible.map(item=>{
      ...
      const playerName=String(player.name||item.playerName||'Player'),hidden=blockFilters.name&&!playerName.toLowerCase().includes(blockFilters.name.toLowerCase());
      return `<article class="live-block-player-row" style="${teamStyle(item.teamKey)}" data-live-block-player-name="${esc(playerName.toLowerCase())}" ${hidden?'hidden':''} data-live-open-player="${esc(id)}" role="button" tabindex="0"><span class="live-block-player-row__brand">${teamMark(item.teamKey)}</span>${image?`<img class="live-block-player-row__photo" src="${esc(image)}" alt="">`:`<span class="live-block-player-row__photo live-block-player-row__photo--empty">${esc(player.position||'P')}</span>`}<button class="live-block-player-row__identity" data-live-open-player-button="${esc(id)}"><strong>${esc(playerName)}</strong><small>${esc(teamName(item.teamKey))}</small></button><div class="live-block-player-row__metrics"><span><small>POS</small><strong>${esc(player.position||'—')}</strong></span><span><small>OVR</small><strong>${esc(player.overall??'—')}</strong></span><span><small>DEV</small><strong>${esc(player.dev||player.developmentTrait||'Normal')}</strong></span><span><small>AGE</small><strong>${esc(player.age??'—')}</strong></span><span><small>CAP HIT</small><strong>${esc(money(player.capHit??player.contract?.capHit))}</strong></span></div>${market}<div class="live-block-actions">${ownListing?`<button class="icon-button is-active" data-live-request-block-remove="${esc(id)}" aria-label="Remove ${esc(player.name||'player')} from the Trade Block" title="Remove from Trade Block">★</button>`:`<button class="button button--primary" data-live-start-block-type="player" data-live-start-block-id="${esc(id)}">Add to Trade</button>`}</div></article>`;
    }).join('');
    if(blockManagerOpen){
  ...
  function filterTradeBlockByName(value){
    const term=String(value||'').toLowerCase();
    blockFilters.name=String(value||'');
    let matches=0;
    document.querySelectorAll('[data-live-block-player-name]').forEach(row=>{
      const visible=!term||String(row.dataset.liveBlockPlayerName||'').includes(term);
      row.hidden=!visible;
      if(visible)matches+=1;
    });
    const empty=document.querySelector('[data-live-block-filter-empty]');
  ...
  document.addEventListener('click',event=>{
    ...
    if(target=event.target.closest('[data-live-arm-management]')){event.preventDefault();managementArmedAction=target.dataset.liveArmManagement;window.FGC_TRADE?.renderCommissioner?.();return}
    if(target=event.target.closest('[data-live-cancel-management]')){event.preventDefault();managementArmedAction=null;window.FGC_TRADE?.renderCommissioner?.();return}
    if(target=event.target.closest('[data-live-confirm-management]')){event.preventDefault();const action=target.dataset.liveConfirmManagement;request(action,{}).then(()=>{managementArmedAction=null;showToast(action==='reset-all-trades'?'Trade history cleared':'Season trade counters reset',action==='reset-all-trades'?'Trade Center history is clear. Madden rosters and draft-pick ownership were preserved.':'Every team now has the full configured season allowance.');window.FGC_TRADE?.renderCommissioner?.()}).catch(error=>showToast('Trade management not completed',error.message));return}
