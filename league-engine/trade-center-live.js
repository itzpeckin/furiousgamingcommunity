(() => {
  ...
  let state=null,loading=null,lastError=null,builder=null,blockLookup=new Set(),blockManagerOpen=false,blockRosterOpen=false,blockEditor=null,blockRosterSearch='',cancelArmedTradeId=null;
  ...
  function assetRow(asset){
    if(asset.assetType==='player'){
      ...
      return `<div class="live-trade-asset live-trade-asset--player">${image?`<img src="${esc(image)}" alt="">`:'<span class="trade-package-asset__placeholder">P</span>'}<span><button type="button" class="text-button" data-live-open-player-button="${esc(id)}">${esc(assetLabel(asset))}</button><small>${esc(player.position||'—')} · ${esc(player.overall??'—')} OVR · ${esc(teamAbbr(asset.fromTeamKey))} → ${esc(teamAbbr(asset.toTeamKey))}</small></span>${valueMarkup(asset)}</div>`;
    }
    return `<div class="live-trade-asset live-trade-asset--pick"><span class="trade-pick-ticket">PICK</span><span><strong>${esc(assetLabel(asset))}</strong><small>${esc(teamAbbr(asset.fromTeamKey))} → ${esc(teamAbbr(asset.toTeamKey))}</small></span>${valueMarkup(asset)}</div>`;
  }
  ...
  function participantAssetPicker(teamKey){
    ...
    return `<article class="trade-builder-team" style="${teamStyle(teamKey)}"><header class="trade-builder-team__hero"><span class="trade-builder-team__mark">${teamMark(teamKey)}</span><div><small>${esc(visual.abbr)}</small><h2>${esc(teamName(teamKey))}</h2></div><dl><div><dt>Cap Space</dt><dd>${esc(teamCapSpace(teamKey))}</dd></div><div><dt>Team Needs</dt><dd>${esc(needs.slice(0,3).join(', ')||'Open')}</dd></div></dl>${teamKey!==currentTeam()?`<button class="icon-button" data-live-remove-team="${esc(teamKey)}" aria-label="Remove ${esc(teamName(teamKey))}">×</button>`:''}</header><div class="trade-builder-team__tools"><div class="segmented-tabs live-asset-filters" aria-label="${esc(teamName(teamKey))} asset filters">${[['players','Players'],['picks','Picks']].map(([key,label])=>`<button class="${filter===key?'is-active':''}" data-live-asset-filter="${key}" data-live-filter-team="${esc(teamKey)}">${label}</button>`).join('')}</div><label class="trade-builder-search"><span aria-hidden="true">⌕</span><input data-live-asset-search="${esc(teamKey)}" value="${esc(search)}" placeholder="Search ${esc(teamName(teamKey))} ${filter}…" aria-label="Search ${esc(teamName(teamKey))} ${filter}"></label></div><div class="live-trade-picker-list">${playerRows}${pickRows||(!playerRows?'<div class="empty-mini">No matching assets.</div>':'')}</div></article>`;
  }
  ...
  function renderDetail(workflow){
    ...
    page().innerHTML=`<div class="trade-detail-page"><div class="page-heading"><div><button class="text-button" data-live-trade-back>← Trade Center</button><span class="eyebrow">${esc(statusLabel(workflow.status))}</span><h1>${esc(workflow.participants.map(item=>teamAbbr(item.teamKey)).join(' ↔ '))}</h1>${workflow.note?`<p>${esc(workflow.note)}</p>`:''}</div><div class="heading-actions">${action}</div></div><div class="trade-package-matchup trade-package-matchup--detail">${workflow.participants.map(item=>{const incoming=workflow.assets.filter(asset=>asset.toTeamKey===item.teamKey);return `<section class="trade-package-team" style="${teamStyle(item.teamKey)}"><div class="trade-package-team__watermark">${teamMark(item.teamKey)}</div><header>${teamMark(item.teamKey)}<strong>${esc(teamAbbr(item.teamKey))} RECEIVES</strong></header><div>${incoming.map(assetRow).join('')||'<div class="empty-mini">No incoming assets</div>'}</div></section>`}).join('<span class="trade-package-swap" aria-hidden="true">⇄</span>')}</div>${fairnessPanel(workflow.assets,workflow.participants.map(item=>item.teamKey))}${review}${privateDetails?`<article class="card live-trade-messages"><div class="card-header"><div><h3>Negotiation</h3><p>Shared only with participating owners and authorized reviewers.</p></div></div>${workflow.messages.map(message=>`<div class="live-trade-message"><strong>${esc(message.authorName||'System')}</strong><p>${esc(message.message)}</p><small>${esc(date(message.createdAt))}</small></div>`).join('')||'<div class="empty-mini">No messages yet.</div>'}${participant&&!['approved','rejected','withdrawn'].includes(workflow.status)?`<div class="live-trade-message-compose"><textarea data-live-message-text placeholder="Write a message…"></textarea><button class="button button--primary" data-live-send-message="${esc(workflow.id)}">Send</button></div>`:''}</article>`:''}</div>`;
  }
  ...
  function renderTradeBlock(){
    ...
    const cards=visible.map(item=>{const player=item.player||{},image=playerImage(player),needs=teamNeeds(item.teamKey),id=player.id||item.playerPublicId||'';return `<article class="live-block-player-row" style="${teamStyle(item.teamKey)}" data-live-open-player="${esc(id)}" role="button" tabindex="0"><span class="live-block-player-row__brand">${teamMark(item.teamKey)}</span>${image?`<img class="live-block-player-row__photo" src="${esc(image)}" alt="">`:`<span class="live-block-player-row__photo live-block-player-row__photo--empty">${esc(player.position||'P')}</span>`}<button class="live-block-player-row__identity" data-live-open-player-button="${esc(id)}"><strong>${esc(player.name||item.playerName||'Player')}</strong><small>${esc(teamName(item.teamKey))}</small></button><div class="live-block-player-row__metrics"><span><small>POS</small><strong>${esc(player.position||'—')}</strong></span><span><small>OVR</small><strong>${esc(player.overall??'—')}</strong></span><span><small>DEV</small><strong>${esc(player.dev||player.developmentTrait||'Normal')}</strong></span><span><small>AGE</small><strong>${esc(player.age??'—')}</strong></span><span><small>CAP HIT</small><strong>${esc(money(player.capHit??player.contract?.capHit))}</strong></span></div><div class="live-block-player-row__market"><small>Looking for</small><strong>${esc(item.requestedReturn||'Open to offers')}</strong>${needs.length?`<span>${needs.slice(0,3).map(need=>`<em>${esc(need)}</em>`).join('')}</span>`:''}</div><div class="live-block-actions">${item.teamKey===own?`<button class="icon-button is-active" data-live-edit-block="${esc(id)}" aria-label="Manage ${esc(player.name||'player')} listing">★</button>`:`<button class="button button--primary" data-live-start-block-type="player" data-live-start-block-id="${esc(id)}">Add to Trade</button>`}</div></article>`}).join('');
    if(blockManagerOpen){
  ...
  window.addEventListener('franchisehq:league-tenant-changed',()=>{state=null;builder=null;blockManagerOpen=false;blockRosterOpen=false;blockEditor=null;blockRosterSearch='';cancelArmedTradeId=null;blockLookup.clear();assetFilters.clear();assetSearch.clear()});
