(() => {
  ...
  function renderTeamDetail(teamId) {
    ...
    pageContent.innerHTML = `
      <div class="page-heading"><div><button class="text-button" data-route="teams"><svg style="transform:rotate(180deg)"><use href="#icon-arrow"></use></svg>All teams</button></div><div class="heading-actions"><button class="button button--ghost" data-demo-toast="Team-follow notifications will be connected with Discord preferences later."><svg><use href="#icon-bell"></use></svg>Follow team</button><button class="button button--primary" data-route="trade-center"><svg><use href="#icon-swap"></use></svg>Trade with ${team.abbr}</button></div></div>
  ...
  function renderRosterTable(roster) {
    return `<article class="card"><div class="card-header"><div><span class="eyebrow">53-player mock roster</span><h3>Active roster</h3></div><span class="pill pill--neutral">${roster.length} records</span></div><div class="table-wrap"><table><thead><tr><th>Player</th><th>OVR</th><th>Age</th><th>Development</th><th>Contract</th><th>Cap Hit</th><th>Status</th></tr></thead><tbody>${roster.map(player => `<tr class="clickable-row" data-player-id="${player.id}"><td>${renderPlayerIdentity(player,false)}</td><td><span class="rating-chip ${player.overall>=90?'rating-chip--elite':player.overall>=84?'rating-chip--high':''}">${player.overall}</span></td><td>${player.age}</td><td><span class="dev-badge ${devClass(player.dev)}">${player.dev}</span></td><td>${player.years} year${player.years===1?'':'s'}</td><td>${formatMoney(player.capHit)}</td><td><span class="pill ${player.injury==='Healthy'?'pill--success':'pill--warning'}">${player.injury}</span></td></tr>`).join('')}</tbody></table></div></article>`;
  }
  ...
  function renderPlayerProfile(playerId) {
    ...
    pageContent.innerHTML = `
      <div class="page-heading"><div><button class="text-button" data-route="players"><svg style="transform:rotate(180deg)"><use href="#icon-arrow"></use></svg>Player database</button></div><div class="heading-actions"><button class="button button--ghost" data-demo-toast="Player watchlists will be saved to each Discord account later."><svg><use href="#icon-star"></use></svg>Watch player</button><button class="button button--primary" data-route="trade-center"><svg><use href="#icon-swap"></use></svg>Add to trade</button></div></div>
