...
`;
  ...
  async function loadLiveTeamDirectory(force=false) {
    ...
    try{
      ...
      // 5.9.10.6.5.4h-p5c — a new activated snapshot must invalidate BOTH layers:
...
function canonicalPlayerDashboardStats(playerId='') {
  ...
  const PERFORMANCE_CERTIFICATION_RELEASE='5.9.10.6.5.4h-p5c';
  ...
  function syncVisibleReleaseMarker() {
    document.querySelectorAll('.version-label,[data-current-release]').forEach(node => {
      node.textContent = 'Current Release - 5.9.10.6.5.4h-p5c';
    });
    document.documentElement.dataset.franchiseHqRelease = '5.9.10.6.5.4h-p5c';
  }
...
/* 5.9.10.6.5.4h-p5c — Historical Roster Released Player Resolver */
(() => {
