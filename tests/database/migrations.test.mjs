...
test('the production-like legacy upgrade preserves identities and relationships', async () => {
  ...
  try {
    ...
    assert.equal(database.prepare('SELECT COUNT(*) count FROM schema_migrations').get().count, 41);
...
test('runtime schema verification fails closed before version 41', async () => {
  ...
  const current = {
    prepare() {
      return { first: async () => ({ version: 41, name: 'commissioner_roster_ownership' }) };
    }
  };
  assert.equal((await requireDatabaseSchema(current)).version, 41);
});
