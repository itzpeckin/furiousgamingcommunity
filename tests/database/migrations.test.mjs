...
test('the production-like legacy upgrade preserves identities and relationships', async () => {
  ...
  try {
    ...
    assert.equal(database.prepare('SELECT COUNT(*) count FROM schema_migrations').get().count, 42);
...
test('runtime schema verification fails closed before version 42', async () => {
  ...
  const current = {
    prepare() {
      return { first: async () => ({ version: 42, name: 'yearly_schedule_import' }) };
    }
  };
  assert.equal((await requireDatabaseSchema(current)).version, 42);
});
