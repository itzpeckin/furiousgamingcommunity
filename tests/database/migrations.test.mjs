...
test('the production-like legacy upgrade preserves identities and relationships', async () => {
  ...
  try {
    ...
    assert.equal(database.prepare('SELECT COUNT(*) count FROM schema_migrations').get().count, 45);
...
test('runtime schema verification fails closed before version 45', async () => {
  ...
  await assert.rejects(
    () => requireDatabaseSchema(outdated),
    error => error?.code === 'DATABASE_MIGRATION_REQUIRED' && error?.currentVersion === 36
  );
  observedVersion = 37;
  await assert.rejects(
    () => requireDatabaseSchema(outdated),
  ...
  observedVersion = 39;
  await assert.rejects(() => requireDatabaseSchema(outdated), error => error?.currentVersion === 39);
  observedVersion = 40;
  await assert.rejects(() => requireDatabaseSchema(outdated), error => error?.currentVersion === 40);
  observedVersion = 42;
  await assert.rejects(() => requireDatabaseSchema(outdated), error => error?.currentVersion === 42);
  observedVersion = 43;
  await assert.rejects(() => requireDatabaseSchema(outdated), error => error?.currentVersion === 43);
  observedVersion = 44;
  await assert.rejects(() => requireDatabaseSchema(outdated), error => error?.currentVersion === 44);

  const current = {
    prepare() {
      return { first: async () => ({ version: 45, name: 'discord_delivery_diagnostics' }) };
    }
  };
  assert.equal((await requireDatabaseSchema(current)).version, 45);
});
