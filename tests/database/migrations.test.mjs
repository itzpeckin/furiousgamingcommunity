...
test('the production-like legacy upgrade preserves identities and relationships', async () => {
  ...
  try {
    ...
    assert.equal(database.prepare('SELECT COUNT(*) count FROM schema_migrations').get().count, 46);
...
test('runtime schema verification fails closed before version 46', async () => {
  ...
  const current = {
    prepare() {
      return { first: async () => ({ version: 46, name: 'platform_league_onboarding' }) };
    }
  };
  assert.equal((await requireDatabaseSchema(current)).version, 46);
});
