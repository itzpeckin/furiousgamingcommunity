...
test('the production-like legacy upgrade preserves identities and relationships', async () => {
  ...
  try {
    ...
    assert.equal(database.prepare('SELECT COUNT(*) count FROM schema_migrations').get().count, 32);
    assert.equal(database.prepare('PRAGMA foreign_key_check').all().length, 0);
  } finally {
    database.close();
  }
...
test('runtime schema verification fails closed before version 31', async () => {
  ...
  observedVersion = 31;
  assert.equal((await requireDatabaseSchema(outdated)).version, 31);

  const current = {
    prepare() {
      return { first: async () => ({ version: 31, name: 'canonical_transaction_corrections' }) };
    }
  };
  assert.equal((await requireDatabaseSchema(current)).version, 31);
});
