...
test('the production-like legacy upgrade preserves identities and relationships', async () => {
  ...
  try {
    ...
    assert.equal(database.prepare('SELECT COUNT(*) count FROM schema_migrations').get().count, 34);
    assert.equal(database.prepare('PRAGMA foreign_key_check').all().length, 0);
  } finally {
    database.close();
  }
...
test('runtime schema verification fails closed before version 34', async () => {
  ...
  observedVersion = 34;
  assert.equal((await requireDatabaseSchema(outdated)).version, 34);

  const current = {
    prepare() {
      return { first: async () => ({ version: 34, name: 'discord_bot_foundation' }) };
    }
  };
  assert.equal((await requireDatabaseSchema(current)).version, 34);
});
