...
test('canonical migration loading includes Trade Block team profiles', async () => {
  const { contract, migrations } = await loadCanonicalMigrations();
  assert.equal(contract.currentVersion, 30);
  assert.equal(migrations.at(-1)?.version, 30);
  assert.equal(migrations.at(-1)?.relativePath, 'migrations/0030_trade_block_team_profiles.sql');
