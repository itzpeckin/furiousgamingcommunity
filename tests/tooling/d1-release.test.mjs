...
test('canonical migration loading includes transaction corrections', async () => {
  const { contract, migrations } = await loadCanonicalMigrations();
  assert.equal(contract.currentVersion, 31);
  assert.equal(migrations.at(-1)?.version, 31);
  assert.equal(migrations.at(-1)?.relativePath, 'migrations/0031_canonical_transaction_corrections.sql');
