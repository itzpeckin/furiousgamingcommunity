...
test('canonical migration loading includes Commissioner HQ Rules', async () => {
  const { contract, migrations } = await loadCanonicalMigrations();
  assert.equal(contract.currentVersion, 32);
  assert.equal(migrations.at(-1)?.version, 32);
  assert.equal(migrations.at(-1)?.relativePath, 'migrations/0032_commissioner_hq_rules.sql');
