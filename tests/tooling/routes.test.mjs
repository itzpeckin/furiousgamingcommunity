...
test('production shell exposes the exact release and environment', async () => {
  ...
  assert.match(index, /franchise-hq-platform-version" content="7\.5\.1"/);
  assert.match(index, /Candidate · Release 7\.5\.1/);
  assert.match(index, /app\.js\?v=7\.5\.1/);
  assert.match(index, /trade-module\.js\?v=7\.5\.1/);
  assert.match(app, /const VISIBLE_RELEASE = '7\.5\.1'/);
  assert.match(app, /hostname==='franchisehq\.app'.*return 'Production'/);
  for (const source of [landing, selector, leagueRoute]) {
    assert.match(source, /const RELEASE ?= ?["']7\.5\.1["']/);
  }
});
