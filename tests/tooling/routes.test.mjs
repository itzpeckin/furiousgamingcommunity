...
test('maps Pages Function file names to stable route patterns', () => {
  ...
  assert.equal(
    functionRoutePath('functions/api/leagues/[leagueSlug]/commissioner-hq.js'),
    '/api/leagues/:leagueSlug/commissioner-hq'
  );
  assert.equal(
    functionRoutePath('functions/api/leagues/[leagueSlug]/discord.js'),
    '/api/leagues/:leagueSlug/discord'
  );
  assert.equal(
    functionRoutePath('functions/api/leagues/[leagueSlug]/news.js'),
    '/api/leagues/:leagueSlug/news'
  );
  assert.equal(
    functionRoutePath('functions/api/discord/interactions.js'),
...
test('production shell exposes the exact release and environment', async () => {
  ...
  assert.match(index, /franchise-hq-platform-version" content="7\.6\.0-rc\.1"/);
  assert.match(index, /Private RC · Release 7\.6\.0-rc\.1/);
  assert.match(index, /app\.js\?v=7\.6\.0-rc\.1/);
  assert.match(index, /trade-module\.js\?v=7\.6\.0-rc\.1/);
  assert.match(app, /const VISIBLE_RELEASE = '7\.6\.0-rc\.1'/);
  assert.match(app, /hostname==='franchisehq\.app'.*return 'Production'/);
  for (const source of [landing, selector, leagueRoute]) {
    assert.match(source, /const RELEASE ?= ?["']7\.6\.0-rc\.1["']/);
  }
});
