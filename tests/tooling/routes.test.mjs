...
test('production shell exposes the exact release and environment', async () => {
  ...
  assert.match(index, /franchise-hq-platform-version" content="8\.0\.8\.1"/);
  assert.match(index, /Production · Release 8\.0\.8\.1/);
  assert.match(index, /app\.js\?v=8\.0\.8\.1/);
  assert.match(index, /trade-module\.js\?v=8\.0\.8\.1/);
  assert.match(app, /const VISIBLE_RELEASE = '8\.0\.8\.1'/);
