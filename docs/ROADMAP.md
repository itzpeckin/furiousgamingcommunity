...
**Current production:** 7.4.0.5 is active from exact Main merge `0c372bd`; the live public and protected-route release markers were verified on September 4, 2026. Migration 30 is the current Production database boundary. No migration, league-data operation, or league-specific draft-pick baseline ran. Blocked Madden Free Agents remain unknown/null and are not interpreted as zero.

**Current work:** 7.4.0.6 is a locally validated code-only candidate. It adds proposer revision of sent negotiating offers, premium opened-proposal asset rows, preserved multi-team selection position, direct owned-listing removal, exact-player notes focus, and consolidated mobile Trade Center/Trade Block remediation. Production remains 7.4.0.5.

**Next gate:** Obtain exact-commit authorization to publish 7.4.0.6, open and validate its pull request, merge it to Main, and deploy the code-only release to Production. No migration or data operation is required. Do not seed picks, import, reset, delete, archive, rotate, transition, change the active snapshot, or reinterpret blocked Free Agents as zero.
...
| 7.4.0.6 | Locally validated candidate | Sent-offer revision, premium proposal details, stable multi-team selection position, direct Trade Block editing/removal, and consolidated mobile remediation |
