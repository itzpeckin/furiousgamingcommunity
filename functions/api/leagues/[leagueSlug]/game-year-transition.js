...
function datasetQueries(leagueId, gameYearId) {
  ...
  return {
    ...
    league_player_ownership:[`SELECT * FROM league_player_ownership WHERE league_id=? AND source_snapshot_id IN (${snapshotScope}) ORDER BY player_identity_id`, [leagueId,leagueId,gameYearId]],
...
async function removeActiveData(current, gameYear, transition, body) {
  ...
  const statements = [
    ...
    ...deleteWhere(current.db,'league_player_ownership','source_snapshot_id',snapshotIds),
