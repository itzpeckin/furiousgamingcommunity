import { database, json, canonicalLeagueSlug, resolveLeague, validLeagueSlug } from '../_lib/cloud-platform.js';
...
export async function onRequestGet(context) {
  try {
    ...
    const slug = canonicalLeagueSlug(url.searchParams.get('slug'));
    if (!validLeagueSlug(slug)) return json({ok:false,error:'A valid league slug is required.'},400);
    ...
    return json({ok:true,release:'7.3.0',league:{
      ...
      discordConnected:Boolean(league.discord_connected),publicStatus:league.public_status,
      tenantStatus:league.tenant_status,timezone:league.timezone,branding:league.branding,
      features:league.features,domains:league.domains,createdAt:league.created_at,updatedAt:league.updated_at
    }});
  } catch (error) {
