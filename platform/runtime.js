(() => {
  ...
  register({
    ...
    routes: [],
    permissions: [],
    dependencies: ['service:contract', 'service:events', 'service:state', 'service:errors', 'service:api'],
    diagnostics: () => ({ services: HQ.listServices?.().filter((name) => ['contract', 'events', 'state', 'errors', 'api'].includes(name)) })
  });
